...
public class PaymentWebhookService {
    ...
    public void processPayment(WebHookNotificationData body) {
        ...
        if (mpPayment == null){
            paymentEventService.save(paymentEntity, paymentId, PaymentEventType.IGNORED, null, null, "payment not found");
            return;
        }
        ...
        if (!"approved".equals(statusFromMp)) {
            paymentEventService.save(paymentEntity, paymentId, PaymentEventType.IGNORED, statusFromMp, null, null);
            return;
        }

        try {
            paymentConfirmationService.verifyPayment(mpPayment);
            paymentEventService.save(paymentEntity, paymentId, PaymentEventType.PROCESSED, statusFromMp,statusDetailFromMp, null);
        } catch (Exception e) {
            paymentEventService.save(paymentEntity, paymentId, PaymentEventType.ERROR, statusFromMp,statusDetailFromMp, e.getMessage());
        }
    }
